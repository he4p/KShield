...
class AppHandler(BaseHTTPRequestHandler):
    ...
    def _check_auth(self, path: str) -> bool:
        public_paths = {"/healthz", "/login", "/", "/style.css", "/app.js", "/auth.js"}
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if path == "/api/v1/events":
            ...
            self._json(
                HTTPStatus.OK, {"items": self.app.storage.list_events(limit=limit)}
            )
            return
        if path == "/api/v1/detections":
            limit = as_int(query.get("limit", ["200"])[0], 200)
            self._json(
                HTTPStatus.OK, {"items": self.app.storage.list_detections(limit=limit)}
            )
            return
